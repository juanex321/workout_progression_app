...
def _reconcile_logged_sessions(db: OrmSession, active_session_id: int) -> None:
    ...
    for session in sessions:
        ...
        if exercise_sessions and all(exercise_session.completed for exercise_session in exercise_sessions):
            session.completed = 1
            changed = True
    if changed:
        db.commit()
...
def get_myo_session_by_number(
    ...
    """Return the exact numbered Myo session for read-only review."""
    sessions = _ordered_myo_sessions(db)
    ...
    if myo_session.id == sessions[-1].id and not myo_session.completed:
